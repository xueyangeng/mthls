 ...
 * @param {boolean} seeking - 是否是seeking
...
export function fragmentWithinToleranceTest(
  ...
  // 不是第一片 并且seektime大于当前片end - 最大容错candidateLookupTolerance设为0 去找上一片
  if (
    candidate?.sn &&
    isSeeking &&
    bufferEnd > (candidate?.endPTS ?? 0) - candidateLookupTolerance
  ) {
    candidateLookupTolerance = 0;
  }
  if (
